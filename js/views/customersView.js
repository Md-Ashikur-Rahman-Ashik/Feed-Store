            ...
            <div class="flex items-start justify-between gap-2">
                <div class="flex items-center gap-3 min-w-0 flex-1">
                    <div class="w-8 h-8 rounded-full ${hasDues ? "bg-red-100" : "bg-amber-100"} flex items-center justify-center flex-shrink-0">
                        <i data-lucide="user" class="w-4 h-4 ${hasDues ? "text-red-500" : "text-amber-600"}"></i>
                    </div>
                    <div class="min-w-0">
...
function renderCount() {
  ...
  el.innerHTML = `<i data-lucide="users" class="w-3 h-3 inline-block align-text-bottom text-stone-400 mr-0.5"></i> ${n === 1 ? "1 customer" : `${n} customers`}`;
  if (window.lucide) lucide.createIcons();
}
